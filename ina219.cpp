...
INA219::INA219()
{
	 std::cout<<"Try to set default configuration to INA219"<<std::endl;	
	init_i2c(__ADDRESS);
    // set the following paramters based on DFRobot_INA219
	_shunt_ohms = 0.01;
	_max_expected_amps = 8;
	_min_device_current_lsb = __CALIBRATION_FACTOR / (_shunt_ohms * __MAX_CALIBRATION_VALUE);
        //configure(RANGE_16V, GAIN_8_320MV, ADC_10BIT, ADC_10BIT);
}
...
INA219::INA219(float shunt_resistance, float max_expected_amps)
{
	...
	calValue = (uint16_t)((111 / 108.5) * calValue) & 0xFFFE;
	write_register(__REG_CALIBRATION, calValue);
}
...
{
	//chosse i2c-0 or i2c-1 (default) depending on system setting
